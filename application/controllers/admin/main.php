...
//        $data['office'] = $this->get_office_specific($username);
...
// EMPLOYEE Controllers
    ...
    public function add_emp(){
        ...
        $data['office'] = $this->main_model->get_office_all();
    ...
    public function upload_file_emp(){
        ...
        $office = $this->input->post_get('office');
        // $this->load->model('admin/main_model');
        $data = $this->main_model->add_emp($params);
        $this->main_model->add_user($empid, $office);
...
//    Attendance Controllers
    ...
    public function load_attendance_table(){
        $username = $this->input->post('username');
        $office = $this->get_office($username);
        $data['results'] = $this->main_model->get_attendance_info($office);
...
//    Scheduling Template
    ...
    public function load_scheduling_template(){
        ...
        $data['templates'] = $this->main_model->get_schedule_template_all($office);
...
//        print_r($data);
    ...
    public function add_template(){
        ...
        // $this->load->model('admin/main_model');
        $data = $this->main_model->save_template($ip,$dsc,$timein,$timeout,$dept,$color,$office);
        print_r($data);
    }
...
//    Godaddy API
    ...
    public function load_it_godaddy_domains(){
        $empid =  $this->session->userdata('username');
        $data['accounts'] = $this->main_model->get_it_godaddy();
        $data['results'] = $this->main_model->get_it_godaddy_domains();
//        $data['read_status'] = $this->main_model->get_it_report_read_status($empid);
        $this->load->view('admin/it_godaddy', $data);
    }

    public function getGDAccountInfo(){
        $account = $this->input->post_get('account');


//        $account = "kambojapoipet";
        $result = $this->main_model->get_it_godaddy_accounts_api($account);


        $api_key = $result[0]->api_key;
        $secret_key = $result[0]->secret_key;

        $this->main_model->delete_gd_accounts_credentials($account);

        $headers = [
            'Authorization:'.'sso-key '.$api_key.':'.$secret_key
        ];

        $ch = curl_init("https://api.godaddy.com/v1/domains?limit=500");
        curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER,false);
        curl_setopt($ch, CURLOPT_HTTPHEADER,$headers);
...
//            $ip = $this->getCFIPDetails($email, $api_key, $row->id);
            ...
            $arrayList = array(
                ...
                "expires" => $row->expires,
                "privacy" => $row->privacy,
                "renewAuto" => $row->locked,
                "locked" => $row->locked

            );
            $this->db->insert('it_godaddy_domains',$arrayList );
        }
...
//        $server = $this->input->post_get('server');
        $serverid = 1;
        $data['serverinfo'] = $this->main_model->get_it_server($serverid);


        $host = $data['serverinfo'][0]->host;
        $hash = $data['serverinfo'][0]->access_hash;
        $user = $data['serverinfo'][0]->user;



        $data['loadave'] = $this->getCPBandwidth($host, $hash, $user);
        $data['diskusg'] = $this->getCPDiskUsage($host, $hash, $user);

        print_r( $data['diskusg']);
        $this->load->view('admin/server_info',$data);

    }

    public function getCPBandwidth($host, $hash, $whmusername){

        $query = "https://".$host.":2087/json-api/loadavg?api.version=1";
    ...
    public function getCPDiskUsage($host, $hash, $whmusername){

        $query = "https://".$host.":2087/json-api/getdiskusage?api.version=1";
